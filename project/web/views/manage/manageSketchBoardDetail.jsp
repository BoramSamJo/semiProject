	...
	<section id="notiSection">
		...
		<div id="readReply">
			...
			<table id="replyTable">
				...
				<tr id="replyTr">
					<td id="replyView" colspan="2"><input type="hidden" id="getRno"
						value="<%=rList.get(i).getrNo()%>"> <input type="hidden"
						id="getMno" value="<%=rList.get(i).getMemberNo()%>"><%=rList.get(i).getrContent()%></td>
					<!-- <td align="right">
						<button type="button" class="updateModifyBtns" id="modifyBtn">수정</button>
					</td>
 -->				<td align="right">
						<button type="button" class="updateDeleteBtns" id="deleteBtn">삭제</button>
					</td>
				</tr>
				<%
					}
		...
		<button id="list"
			onclick="location.href='<%=request.getContextPath()%>/manageList.ms'">글목록</button>
			
		<button id="delete" type="button">글삭제</button>
	...
	<script>
          ...
          $(function(){
                ...
                var sbNo = <%=sb.getSbNo()%>;
                location.href='<%=request.getContextPath()%>/deleteList.ms?sbNo='+sbNo;
                console.log(sbNo);
                })
           });
            ...
            $("button[type='button']").click(function(){
    			...
    			if(bool){
    				location.href="<%=request.getContextPath()%>/deleteReply.ms?rNo="+rNo+"&sbNo="+sbNo;
    			}}
            });
