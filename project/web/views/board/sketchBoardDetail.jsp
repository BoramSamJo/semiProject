...
#replyTable tr:nth-of-type(3) td:nth-of-type(3) {
	width: 300px;
}
...
<body>
	...
	<section id="notiSection">
		...
		<div id="readReply">
			...
			<table id="replyTable">
				<tr>
					<td colspan="3"><img src="<%=request.getContextPath()%>/img/talk.png">&nbsp;<span>댓글</span></td>
				</tr>
				<%
					if (rList != null) {
				%>
				<%
					...
					%>
					<tr id="nameTr">
						<td colspan="3" id="nameView"><img src="<%=request.getContextPath() %>/img/user.png">&nbsp;<%=rList.get(i).getMemberName()%></td>
					</tr>
					<tr id="replyTr">
						...
						<td align="right">
							<button style="margin:0px; width:40px;" type="button" class="updateModifyBtns" id="modifyBtn">수정</button>
							<button style="margin:0px; width:40px" type="button" class="updateDeleteBtns" id="deleteBtn">삭제</button>
						</td>
					</tr>
					<%
					}
               			...
               			success:function(data){
               				...
               				for(var i=0; i<data.length; i++){
               					if(i==0){
               						...
               						var $tr3 = $("<tr>");
               						var $td3 = $("<td>").text(data[i].rContent).css("width","300px");
               						var hidden = $("<input>").attr({"type":"hidden","value":data[i].rNo,"id":"getRno"});
               						var hidden2 = $("<input>").attr({"type":"hidden","value":data[i].memberNo,"id":"getMno"});
               						var $td4 = $("<td>");
               						var btn1 = $("<button>").attr("type","button").addClass("updateModifyBtns").attr("id","modifyBtn").text("수정");
               						var $td5 = $("<td>");
               						var btn2 = $("<button>").attr("type","button").addClass("updateDeleteBtns").attr("id","deleteBtn").text("삭제");
...
/* 	    			console.log(rNo); */
	           		...
	           		if(bool){
	           			if(window.mNo==mNo2){
	           			var originContent = $.trim($(this).parent().parent().children().eq(0).text());
	           			console.log(originContent);
	                    var changeContent = $("#replyContent").val(originContent);
	           			changeContent.text();
	           			$(changeContent).focus();
	           			$("#replySubmit").click(function(){
	           				
	           				/* var mNo2 = $("#modifyBtn").parent().parent().children().children("#getMno").val(); */
	           				/* var rNo = $(this).parent().parent().children().children("#getRno").val(); */
	       
	           				var changeContent2 = $("#replyContent").val();
	           				
/* 							console.log(mNo2);
	           				console.log(window.sbNo);
	           				console.log(rNo);
