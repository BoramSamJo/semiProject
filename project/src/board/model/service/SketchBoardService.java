...
public class SketchBoardService {
	...
	public ArrayList<SketchBoard> selectList(int currentPage, int limit) {
		...
		ArrayList<SketchBoard> list = new SketchBoardDao().selectList(conn, currentPage, limit);
	...
	public ArrayList<SketchBoard> selectSearchList(int currentPage, int limit, String searchText) {
		Connection conn = getConnection();

		ArrayList<SketchBoard> list = new SketchBoardDao().selectSearchList(conn, currentPage, limit, searchText);
...
//		System.out.println("서비스단에서 댓글삭제 : " + result);
	...
	public ArrayList<SbReply> selectManageReply(int sbNo) {
		Connection conn = getConnection();

		ArrayList<SbReply> rList = new SketchBoardDao().selectManageReply(conn, sbNo);

		close(conn);
		return rList;
	}

	public int manageDeleteList(int sbNo) {
		Connection conn = getConnection();

		SketchBoardDao sbd = new SketchBoardDao();

		int result = sbd.manageDeleteList(conn, sbNo);
	...
	public int searchListCount(String searchText) {
		...
		int result = new SketchBoardDao().searchListCount(conn, searchText);
		;
