...
public class noticeBoardDao {
	...
	public ArrayList selectList(Connection conn, int currentPage, int limit) {
		...
		//0621변경
		String query = "SELECT * FROM(SELECT ROWNUM RNUM, N.* FROM NOTICEBOARD N ORDER BY N_NO DESC) WHERE (RNUM BETWEEN ? AND ?) AND STATUS = 'Y'";
		...
		try {
			...
			while(rs.next()) {
				b = new noticeBoard(rs.getInt("N_NO"),
												rs.getInt("MEMBER_NO"),
	...
	public int insertBoard(Connection conn, noticeBoard nb) {
		...
		String query = "INSERT INTO NOTICEBOARD VALUES((SELECT MAX(N_NO)+1 FROM NOTICEBOARD),1,?,?,SYSDATE,SYSDATE,'Y')";
