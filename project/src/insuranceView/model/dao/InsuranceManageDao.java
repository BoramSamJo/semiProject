...
public class InsuranceManageDao {
	...
	public static ArrayList<InsuranceManage> firstUpdate(Connection conn, Pagination p) {
	      ...
	      String query = "SELECT * FROM (SELECT ROWNUM RNUM,I.* FROM INSLIST I WHERE STATUS='Y') WHERE RNUM BETWEEN ? AND ?";
	...
	public static int getListCount(Connection conn) {
		...
		String query = "SELECT COUNT(INS_NO) FROM INSLIST WHERE STATUS='Y'";
	...
	public static int getListCount(Connection conn, String select, String search) {
		...
		String query = "SELECT COUNT(*) FROM INSLIST WHERE "+select+" LIKE '%"+search+"%' AND STATUS='Y'";
	...
	public static ArrayList<InsuranceManage> getInsList(Connection conn, Pagination p, String select, String search) {
	      ...
	      int endRow = currentPage*limit;
	      System.out.println("current : "+currentPage);
	      String query = "SELECT * FROM (SELECT ROWNUM RNUM,I.* FROM INSLIST I WHERE "+select+" LIKE '%"+search+"%' AND STATUS='Y') WHERE RNUM BETWEEN ? AND ?";
	...
	public static int deleteIns(Connection conn, int insNo) {
		PreparedStatement pstmt = null;
		int result = 0;
		
		String query = "UPDATE INS_LIST SET STATUS='N' WHERE INS_NO=?";
